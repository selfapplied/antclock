...
class PhaseLockedTrainer:
    ...
    def step(self, loss: float, zeta_loss: Optional[float] = None) -> Dict[str, Any]:
        ...
        if self.early_stopper is not None and zeta_loss is not None:
            ...
            step_info['should_stop'] = should_stop

        return step_info

    @property
    def timing_stats(self) -> Dict[str, Any]:
        """Get timing statistics for the training session."""
        return {
            'phase_history': self.phase_history,
            'total_steps': len(self.phase_history),
            'current_phase': self.current_phase,
            'early_stops': 0,  # Not implemented yet
            'awareness_accumulations': len(self.phase_history)  # Simple proxy
        }
...
def create_ce_timed_trainer(model: nn.Module, learning_rate: float = 0.01) -> PhaseLockedTrainer:
    """
    Create a CE-timed trainer for a given model.
    ...
    Returns:
        PhaseLockedTrainer configured with Adam optimizer
    """
    import torch.optim as optim
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    return PhaseLockedTrainer(optimizer)