...
class FactorizationComplexBenchmark:
    ...
    def generate_dataset(self, size: int, progress_callback: Optional[Callable[[int], None]] = None) -> Tuple[List[CE3SimplicialPoint], List[int]]:
...
class SimplicialHomologyBenchmark:
    ...
    def generate_dataset(self, size: int, progress_callback: Optional[Callable[[int], None]] = None) -> Tuple[List[CE3SimplicialPoint], List[float]]:
    ...
    def is_toy_solution_possible(self, dataset: Tuple[List[CE3SimplicialPoint], List[float]]) -> bool:
        ...
        for const in [0.01, 0.1, 0.5, 1, 2]:
            simple_pred = invariants.astype(float) * const
            simple_error = np.mean(np.abs(outputs - simple_pred))
            if simple_error < 0.1:  # Much stricter threshold
                return True

        # Rule 4: Simple linear combination (numpy only)
        try:
            # Simple multiple linear regression approximation
            X = np.column_stack([np.ones(len(dimensions)), dimensions, complexities, invariants])
            y = np.array(outputs)

            # Solve X^T X β = X^T y using numpy
            beta = np.linalg.lstsq(X, y, rcond=None)[0]
            pred = X @ beta
            linear_error = np.mean(np.abs(y - pred))
            if linear_error < 0.1:  # Linear combination should not work well
                return True
        except:
            pass  # If linear algebra fails, continue
    ...
    def _compute_betti_number_1(self, complex_data: Dict[str, Any]) -> int:
        """Compute first Betti number (rank of H1) with complex dependencies."""
        ...
        # Count simplices by dimension
        num_vertices = len(set(v for s in simplices for v in s))
        num_edges = sum(1 for s in simplices if len(s) == 2)
        num_triangles = sum(1 for s in simplices if len(s) == 3)
        num_tetrahedra = sum(1 for s in simplices if len(s) == 4)

        # Base Euler characteristic components
        euler_base = num_vertices - num_edges + num_triangles - num_tetrahedra

        # Add complexity-dependent corrections
        # Higher complexity introduces more non-trivial cycles
        complexity_factor = complexity / 10.0
        cycle_correction = int(complexity_factor * (num_edges - num_vertices))

        # Add dimension-dependent scaling
        dimension_factor = max(1, dimension - 1)
        betti_1 = max(0, euler_base + cycle_correction) * dimension_factor

        # Add non-linear effects from prime factors in complexity
        if complexity > 1:
            prime_factors = self._count_prime_factors(complexity)
            betti_1 += prime_factors * 2  # Primes introduce additional cycles

        # Add oscillatory component to prevent simple correlations
        oscillation = int(3 * math.sin(complexity * 0.1))
        betti_1 += oscillation

        return max(0, min(betti_1, 50))  # Cap at reasonable value
