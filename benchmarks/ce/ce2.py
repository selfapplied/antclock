...
class GaussMapConvergenceBenchmark:
    ...
    def generate_dataset(self, size: int, progress_callback: Optional[Callable[[int], None]] = None) -> Tuple[List[CE2FlowPoint], List[int]]:
    ...
    def _sqrt_approximation(self, n: int, terms: int) -> float:
        ...
        for _ in range(terms):
            ...
            if abs(fractional_part) < 1e-10:  # Avoid division by zero
                break
    ...
    def _compute_gauss_orbit(self, x0: float, max_steps: int) -> List[float]:
        ...
        for _ in range(max_steps):
            ...
            # Gauss map: T(x) = 1/x - floor(1/x)
            # Protect against division by zero with safe computation
            try:
                inv_x = 1.0 / x if abs(x) > 1e-10 else float('inf')
                if inv_x == float('inf'):
                    break
                x = inv_x - int(inv_x)
    ...
    def _estimate_invariant_measure(self, orbit: List[float]) -> float:
        ...
        hist, bin_edges = np.histogram(orbit, bins=20, range=(0, 1), density=True)

        # The Gauss measure should have density 1/(π√(x(1-x)))
        # Check how well our empirical distribution matches this
        bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2  # 20 bin centers
        theoretical = [1.0 / (math.pi * math.sqrt(max(x * (1-x), 1e-10))) for x in bin_centers]
...
class FlowFieldIntegrationBenchmark:
    ...
    def generate_dataset(self, size: int, progress_callback: Optional[Callable[[int], None]] = None) -> Tuple[List[CE2FlowPoint], List[float]]:
